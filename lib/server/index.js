...
if (process.env.NODE_ENV === 'production' && process.env.CLOUDINARY_CLOUD_NAME) {
  ...
  const storage = new CloudinaryStorage({
    ...
    params: {
      ...
      public_id: (req, file) => {
        ...
        return 'profile-' + (req.user?.id || req.session?.user?.id || 'unknown') + '-' + uniqueSuffix;
      }
    }
  });
  ...
  const storage = multer.diskStorage({
    ...
    filename: function (req, file, cb) {
      ...
      cb(null, 'profile-' + (req.user?.id || req.session?.user?.id || 'unknown') + '-' + uniqueSuffix + ext);
    }
  });
...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    const sql = `
      INSERT INTO users (username, password, password_hash, usn, firstName, lastName, email, approval_status, registered_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', NOW())
    `;

    db.query(sql, [username, password, hashedPassword, usn, firstName, lastName, email], (err, result) => {
      if (err) {
        console.error("Error inserting user:", err);
...
app.post('/borrow', authenticateUser, (req, res) => {
  ...
  db.query(checkBorrowLimitSql, [user_id], (err, limitResult) => {
    ...
    if (activeCount >= 3) {
      return res.status(400).json({
        message: 'You have reached the borrowing limit of 3 books. Please return a book before borrowing another one.',
...
app.get('/borrowed-books', authenticateUser, (req, res) => {
  const sql = `
    SELECT
      b.*,
      bb.borrow_date,
...
app.post('/return-book', authenticateUser, (req, res) => {
  const { book_id } = req.body;
...
app.get('/borrow-history', authenticateUser, (req, res) => {
  const sql = `
    SELECT b.title, b.author, b.acc_no, bb.borrow_date, bb.expiry_date, bb.returned_at
    FROM borrowed_books bb
    JOIN books b ON bb.book_id = b.id
