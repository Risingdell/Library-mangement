...
router.get('/purchase-requests', (req, res) => {
  ...
  // This endpoint returns marketplace book purchase requests that need admin confirmation
  // These are student-to-seller marketplace transactions awaiting admin verification
  const sql = `
    SELECT
      br.id AS request_id,
      ...
      seller.contact AS seller_contact
    FROM book_requests br
    JOIN used_books_marketplace m ON br.marketplace_book_id = m.id
    JOIN users student ON br.requester_id = student.id
    JOIN users seller ON br.seller_id = seller.id
    WHERE br.status = 'active'
      AND br.admin_status = 'pending'
    ORDER BY br.is_priority_buyer DESC, br.requested_at ASC
  `;
...
router.post('/confirm-book-received', (req, res) => {
  if (!req.session.admin) {
    return res.status(401).json({ message: 'Unauthorized' });
  }
  ...
  const getRequestSql = `
    ...
    FROM book_requests br
    JOIN used_books_marketplace m ON br.marketplace_book_id = m.id
    WHERE br.id = ? AND br.status = 'active' AND br.admin_status = 'pending'
  `;
  ...
  db.query(getRequestSql, [request_id], (err, results) => {
    ...
    db.beginTransaction((err) => {
      ...
      db.query(updateRequestSql, [adminId, request_id], (err) => {
        if (err) {
          return db.rollback(() => {
        ...
        // Step 2: Insert into purchased_books
        const insertPurchasedSql = `
          INSERT INTO purchased_books
          (book_id, student_id, seller_id, request_id, title, author, type, description,
           book_format, file_path, requested_at, confirmed_received_at, confirmed_by_admin)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)
        `;

        const purchasedValues = [
          request.marketplace_book_id,
        ...
        db.query(insertPurchasedSql, purchasedValues, (err) => {
          if (err) {
            return db.rollback(() => {
              console.error('Error inserting purchased book:', err);
              res.status(500).json({ message: 'Failed to record purchase' });
            });
          }

          // Step 3: Update marketplace book to completed
          ...
          db.query(updateMarketplaceSql, [request.marketplace_book_id], (err) => {
            if (err) {
              return db.rollback(() => {
            ...
            // Step 4: Cancel all other pending requests for this book
            const cancelOthersSql = `
              UPDATE book_requests
              SET status = 'cancelled',
                  admin_status = 'rejected',
                  cancelled_at = NOW(),
                  rejection_reason = 'Book already purchased by another student'
...
router.post('/reject-purchase-request', (req, res) => {
  if (!req.session.admin) {
    return res.status(401).json({ message: 'Unauthorized' });
  }
  ...
  db.query(updateSql, [adminId, rejection_reason || 'Rejected by admin', request_id], (err, result) => {
    if (err) {
