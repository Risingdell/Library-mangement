...
router.post('/approve-user', authenticateAdmin, (req, res) => {
  ...
  db.query(checkSql, [user_id], (err, result) => {
    ...
    db.query(updateSql, [admin_id, user_id], async (err) => {
      if (err) {
      ...
      if (user.email) {
        try {
          await sendRegistrationApprovalEmail(user.email, {
          ...
          console.error(`❌ Approval email failed for ${user.username}:`, err.message);
        }
      } else {
        emailStatus = 'no_email';
        console.warn(`⚠️ No email address for user ${user.username}`);
      }

      res.json({
        message: `User "${user.username}" approved successfully. They can now login.`,
        emailStatus,
        emailError
      });
    });
...
router.post('/reject-user', authenticateAdmin, (req, res) => {
  ...
  db.query(checkSql, [user_id], (err, result) => {
    ...
    db.query(updateSql, [admin_id, reason, user_id], async (err) => {
      if (err) {
      ...
      if (user.email) {
        try {
          await sendRegistrationRejectionEmail(user.email, {
            firstName: user.firstName,
            lastName: user.lastName,
            username: user.username,
          ...
          emailStatus = 'failed';
          emailError = err.message;
          console.error(`❌ Rejection email failed for ${user.username}:`, err.message);
        }
      } else {
        emailStatus = 'no_email';
        console.warn(`⚠️ No email address for user ${user.username}`);
...
router.get('/pending-returns', authenticateAdmin, (req, res) => {
  const sql = `
    SELECT
      ...
      bb.returned_at,
      bb.return_status,
      CASE WHEN bb.expiry_date < CURDATE() THEN 1 ELSE 0 END AS is_overdue
    FROM borrowed_books bb
    JOIN books b ON bb.book_id = b.id
    JOIN users u ON bb.user_id = u.id
    WHERE bb.return_status IN ('active', 'pending_return')
    ORDER BY bb.expiry_date ASC
  `;
...
router.post('/approve-return', authenticateAdmin, (req, res) => {
  ...
  // First, get the book_id and user_id from borrowed_books
  const getBookSql = 'SELECT book_id, user_id FROM borrowed_books WHERE id = ? AND return_status = "pending_return"';
  ...
  db.query(getBookSql, [borrow_id], (err, result) => {
    ...
    const { book_id, user_id } = result[0];
    ...
    db.query(
      ...
      (err) => {
        ...
        db.query('UPDATE books SET status = "available" WHERE id = ?', [book_id], (err) => {
          ...
          // Mark branch_book_requests as completed so student can request again
          db.query(
            `UPDATE branch_book_requests SET status = 'completed'
...
// Mark book as received directly by admin (for active borrowed books)
router.post('/mark-returned', authenticateAdmin, async (req, res) => {
  const { borrow_id } = req.body;
  const admin_id = req.admin.id;
  ...
  try {
    ...
    const [rows] = await connection.query(
      'SELECT book_id, user_id, return_status FROM borrowed_books WHERE id = ?',
      [borrow_id]
    );
    ...
    res.json({ success: true, message: 'Book marked as received. Status updated to available.' });
