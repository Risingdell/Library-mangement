...
router.post('/login', (req, res) => {
  ...
  db.query(sql, [username, password], (err, results) => {
    ...
    // Set admin session (express-session will automatically set Set-Cookie header)
    req.session.admin = {
      id: results[0].id,
    ...
    console.log(`✅ Admin ${username} authenticated. Session will be auto-saved with Set-Cookie header.`);

    // Send response immediately - express-session middleware will handle Set-Cookie
    res.json({
...
router.get('/purchase-requests', (req, res) => {
  ...
  const sql = `
    ...
    JOIN users seller ON m.seller_id = seller.id
    WHERE br.status = 'active'
      AND m.status IN ('requested', 'sold')
